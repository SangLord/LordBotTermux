...
* REMAKE OF FADHIL GRAPHY × BRYAN GAY:V × SangLord
...
const { help } = require('./SANGLORD/help')
const { rules } = require('./SANGLORS/rules')
const { bahasa } = require('./SANGLORD/bahasa')
const { donasi } = require('./SANGLORD/donasi')
const { limits } = require('./SANGLORD/limit')
const { bokep } = require('./SANGLORD/bokep.js')
const { ownermenu } = require('./SANGLORD/ownermenu.js')
const { groupmenu } = require('./SANGLORD/groupmenu.js')
const { mediamenu } = require('./SANGLORD/mediamenu.js')
const { nsfwmenu } = require('./SANGLORD/nsfwmenu.js')
const { listmenu } = require('./SANGLORD/listmenu.js')
const { funmenu } = require('./SANGLORD/funmenu.js')
const { makermenu } = require('./SANGLORD/makermenu.js')
...
async function starts() {
	...
	client.on('qr', () => {
		console.log(color('[','white'), color('!','red'), color(']','white'), color('BURUAN SCAN!! Maded By SangLord'))
	})
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const SangLord = ["6283144780782@s.whatsapp.net"]
			...
			const ownerNumber = [
			"6283144780782@s.whatsapp.net"//GANTI NOMOR LU
			]
			premium = [
			"6283144780782@s.whatsapp.net","6283102650464@s.whatsapp.net"//GANTI NOMOR YAG MAU DI PREM, GABISA NYIMPEN DI DATABASE!!
			]
		...
		case 'daftar':
			...
			case 'SangLord':
                ...
                setTimeout( () => {
					...
					setTimeout( () => {*/
					client.sendMessage(from, 'Terimakasih sudah menggunakan *SangLord Bot* \n\n*SEBAGAI PENGGANTI DONASI YUK SUPORT*\nSubscribe https://youtube.com/c/MikazuMZ \nFollow Github https://github.com/SangLord', text, {quoted: mek})
					}, 0)
    				break
/*********PISAH MENU**********/
              case 'ownermenu':
...
/*********END PISAH MENU***********/
           ...
           if (!isUser) return reply(mess.only.userB)
            ...
            break
				...
				case 'owner':
				case 'creator':
                 client.sendMessage(from, {displayname: "+62 831-4478-0782", vcard: vcard}, MessageType.contact, { quoted: mek})
...
//akhir kreator
	   ...
	   case 'coli':
				...
				if (!isUser) return reply(mess.only.userB)
				reply(mess.wait)
				 data = fs.readFileSync('./SangLord/bokep.js');
                 jsonData = JSON.parse(data);
                 randIndex = Math.floor(Math.random() * jsonData.length);
                 randKey = jsonData[randIndex];
		    ...
		    case 'map':
				...
				case 'clearall':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of groupMembers) {
							sendMess(_.jid, `*「 BROADCAST GROUP BY SANGLORD 」*\n\n*Group* : ${groupName}\n${name}\n*Pada Jam*: ${jam}WIB\n\n${body.slice(6)}`)
						}
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ *SANGLORD BROADCAST* ]\n\n${body.slice(4)}`)
						}
						reply('Suksess broadcast')
					}
					break
				...
				default:
                  ...
                  reply(`Maaf Kak *${pushname2}*, Command *${prefix}${command}* Tidak Terdaftar Di Dalam *${prefix}menu!*`)
					...
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[SANGLORDBOT]','Yellow'), 'Command Tidak Terdaftar', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
			console.log('Error : %s', color(e, 'yellow'))
